///scr_draw_InvintoryListAndSelection(tab selection var);
//draw item selection
...
draw_rectangle(
    ...
    display_get_gui_width()*0.37+8+string_width("x 000"), 
...
switch (argument0) {
    case 0:
        var invintoryType = global.itemInventory;
        var invintoryIndex = global.itemIndex;
        break;
    case 1:
        var invintoryType = global.medicineInventory;
        var invintoryIndex = global.medicineIndex;
        break;
    case 2:
        var invintoryType = global.captureInventory;
        var invintoryIndex = global.captureIndex;
        break;
    case 3:
        var invintoryType = global.keyInventory;
        var invintoryIndex = global.keyIndex;
        break;
}
if (invintoryType[# 0, 0] != 0) {
    var i;
    for (i = 0; i < ds_grid_height(invintoryType)/*10*/; i += 1){
        ...
        //Draw item quantity
        draw_text(display_get_gui_width()*0.37, 56 + ((i + 1 - scrollNum) * string_height("I")), "x " + string(amount)); 
        //draw_text(x+292+1, y + ((i + moveList) * 32)+1, string(itemType)); //Draw item type
    }
} else {
    draw_text(display_get_gui_width()*0.05, 56 + ((0 + 1 - scrollNum) * string_height("I")), "Empty");
}
